...
public class DataPosture extends BaseDataActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {//手动调整日期，点击发送网络请求
        ...
        switch (view.getId()){
            ...
            case R.id.bt_data_posture_date_cut://日期减
                ...
                //发出请求,继续查询上一周,并且更新UI
                postureInitdata(requestDateText);
                //处理数据弹出反馈
    ...
    public void postureInitdata(final String dateText){

        final RequestBody requestBody=new FormBody.Builder()
                .add("date",dateText)
                .build();

        HttpUtil.sendOkHttpRequestWithTokenAndBody(InfoSave.getSittingPostureDataUrl,getContext(), requestBody, new Callback() {
            @Override
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Log.d(TAG, "请求坐姿数据onResponse: "+responseData);
                if (response.code()==200){//如果正常返回开始解析
                    ...
                    if (getSittingPostureData.getCode()==0){
                        ...
                        Log.d(TAG, "坐姿数据onResponse: "+"dateText="+dateText+"accuracy="+accuracy+" average="+average+" grade="+grade+""+" a="+a+" b="+b+" c="+c+" d="+d+" e="+e+" f="+f+" g="+g);
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (a<=0&&b<=0&&c<=0&&d<=0&&e<=0&&f<=0&&g<=0){
                                    ...
                                    image_posture_nodata.setVisibility(View.VISIBLE);
                                    ...
                                    image_posture_nodata.setVisibility(View.GONE);
